...
const ProgressBarBase: FC<ProgressBarProps> = ({ value, min = 0, max = 100, bandColor = 'brand', className, progressClassName }) => {
  ...
  return (
    <div
      ...
      aria-valuemin={min}
      aria-valuemax={max}
      className={tm('h-2 w-full overflow-hidden rounded-md bg-quaternary', className)}>
      <div
...
const ProgressBar: FC<ProgressIndicatorWithTextProps> = ({
  ...
  const baseProgressBar = (
    <ProgressBarBase min={min} max={max} value={value} className={className} bandColor={bandColor} progressClassName={progressClassName} />
  )
      ...
      return (
        <div className='w-full relative flex flex-col items-end gap-2'>
          ...
          <div
            ...
            className='absolute -top-2 -translate-x-1/2 -translate-y-full rounded-sm bg-primary_alt px-3 py-2 shadow-lg ring-1 ring-secondary_alt'>
    ...
    case 'bottom-floating':
      return (
        <div className='w-full relative flex flex-col items-end gap-2'>
          ...
          <div
            style={{ left: `${percentage}%` }}
            className='absolute -bottom-2 -translate-x-1/2 translate-y-full rounded-sm bg-primary_alt px-3 py-2 shadow-lg ring-1 ring-secondary_alt'>
