...
- 🔄 **Automatic Fallback**: Uses modern Clipboard API with execCommand fallback
- 🎯 **Optional ID Tracking**: Track multiple copy operations with identifiers
- ⏱️ **Configurable Timeout**: Visual feedback state that auto-resets
- 🔒 **Secure Context Aware**: Handles non-HTTPS environments gracefully
- 📱 **Cross-Browser Compatible**: Works in modern and legacy browsers
...
function BasicExample() {
  ...
  return <button onClick={() => copy('Basic text to copy')}>{copied ? '✅ Copied!' : '📋 Copy'}</button>
}
```
