...
<Meta title='utils/use-breakpoint' />
...
- 🎯 **Tailwind Integration**: Uses default Tailwind CSS breakpoints
- 📱 **Responsive Design**: Perfect for conditional rendering based on screen size
- 🔄 **Real-time Updates**: Automatically updates when viewport changes
- 🖥️ **SSR Safe**: Handles server-side rendering without hydration issues
- 🧹 **Auto Cleanup**: Removes event listeners on unmount
- ⚡ **Performant**: Uses native `matchMedia` API
...
```tsx
const isLarge = useBreakpoint('lg') // ✅ Valid
const isInvalid = useBreakpoint('xs') // ❌ TypeScript error
```
